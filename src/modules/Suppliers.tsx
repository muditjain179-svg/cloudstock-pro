...
const Suppliers: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (import.meta.env.DEV) console.error("Error saving supplier:", error);
      setSubmissionError("Error saving supplier: " + (error.message || "An unknown error occurred"));
                  ...
                  <div className="flex justify-end gap-1">
                    {deleteId === supplier.id ? (
                      <div className="flex items-center gap-2 bg-red-50 p-1 rounded-lg border border-red-100 animate-in fade-in slide-in-from-right-1 duration-200">
                        ...
                        <button onClick={async () => { await deleteDoc(doc(db, 'suppliers', supplier.id)); setDeleteId(null); }} className="px-2 py-1 bg-red-600 text-white text-[9px] rounded font-bold uppercase hover:bg-red-700 transition-all active:scale-95 shadow-sm">Yes</button>
                      </div>
                    ) : (
                      <>
                        <button 
